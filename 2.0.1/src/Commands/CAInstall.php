 ...
 *
 * Command to install CRM Admin package into project which moves lot of file from 'src/Installs' directory to Project
 */
class CAInstall extends Command
{
    ...
    protected $description = 'Install  CRM Admin Package. Generate whole structure for /admin.';
    ...
    {
        try {
            $this->info('CRM Admin installation started...');

            $from = base_path('vendor/kipl/crmadmin/src/Installs');
            $to = base_path();
            ...
            if($this->confirm("Want to set your Database config in the .env file ?", true)) {
                ...
                $db_data['db'] = $this->ask('Database Name', 'your_database_name | crmadmin1');
                $db_data['dbuser'] = $this->ask('Database User', 'your_mysql_password | root');
